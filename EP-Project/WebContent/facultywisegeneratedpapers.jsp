...
{%>
<tr><td><%=lst.getPaperid() %></td><td><%=lst.getCoursename() %></td><td><%=lst.getCoursecode() %><td><%=lst.getGeneratorid() %></td><td><%=cd.getfacultyName(id) %></td><td><form action="viewquestionpaperadmin.jsp" method="post"><input type="hidden" name="paperid" value=<%=lst.getPaperid() %>><input type="submit" value="View this paper"></form></td></tr>
